#include <inc/stdio.h>
#include <inc/error.h>
#include <inc/string.h>

// Handle the extended ASCII code inputed by console
inline static void ext_char_handler(int c);
// Handle the escape sequence inputed by serial (user-terminal)
inline static void esc_seq_handler();

// Move the cursor right
inline static void move_right();
// Move the cursor left
inline static void move_left();
// Move the cursor up
inline static void move_up();
// Move the cursor down
inline static void move_down();

// Flush buffer's [cur, tail) to the displays
// and move the cursor back
inline static void flush_buf();

// Insert char to current cursor
inline static void insert_char(int c);
// Remove current cursor's char
inline static void remove_char();
// Terminate the input
inline static void finish_input();

#define BUFLEN 1024
// current input
static char buf[BUFLEN];
// last input
static char last[BUFLEN];

// Current position of cursor
static int cur;
// Tail of buffer
static int tail;

// cursor of history
static int last_length;

static int echoing;
static int history = 0;

char *
readline(const char *prompt)
{
	int c;

	if (prompt != NULL)
		cprintf("%s", prompt);

	cur = tail = 0;
	echoing = iscons(0);
	while (1) {
		c = getchar();
		if (c < 0) {
			cprintf("read error: %e\n", c);
			return NULL;
		} else if ((c == '\b' || c == '\x7f') && cur > 0) {
			remove_char();
		} else if (c >= ' ' && c <= '~' && tail < BUFLEN-1) {
			// DEL(0x7f) is larger than '~'
			// and it will be inputed when you push
			// 'backspace' in user-terminal
			insert_char(c);
		} else if (c == '\n' || c == '\r') {
			finish_input();
			return buf;
		} else if (c == '\x1b') {
			esc_seq_handler(); // only serial will input esc
		} else if (c > '\x7f') {
			ext_char_handler(c); // only console will input extended ascii
		}
	}
}

inline static void 
flush_buf()
{
	for (int i = cur; i < tail; ++i)
		cputchar(buf[i]);
	for (int i = cur; i < tail; ++i)
		cputchar('\b'); // cursor move back
}

inline static void 
insert_char(int c) 
{
	if (cur == tail) {
		tail++, buf[cur++] = c;
		if (echoing)
			cputchar(c);
	} else { // general case
		memmove(buf + cur + 1, buf + cur, tail - cur);
		buf[cur] = c, tail++;
		if (echoing) 
			flush_buf();
		move_right();
	}
}

inline static void 
remove_char()
{
	if (cur == tail) {
		cur--, tail--;
		if (echoing)
			cputchar('\b'), cputchar(' '), cputchar('\b');
	} else { // general case
		memmove(buf + cur - 1, buf + cur, tail - cur);
		buf[tail - 1] = ' ';
		move_left();
		if (echoing)
			flush_buf();
		tail--;
	}
}

inline static void 
move_left()
{
	if (cur > 0) {
		if (echoing)
			cputchar('\b');
		cur--;
	}
}

inline static void 
move_right()
{
	if (cur < tail) {
		if (echoing)
			cputchar(buf[cur]);
		cur++;
	}
}

inline static void 
move_up()
{
	if (!history){
		memmove(buf, last, BUFLEN);
		cur = tail;
		for (; cur; remove_char());
		// tail = last_length;
		for (int i = cur; i < last_length; ++i)
			cputchar(buf[i]);
		cur = tail = last_length;
		history = 1;
	}
}

inline static void 
move_down()
{
	if (history){
		cur = tail;
		for (; cur; remove_char());
		// insert_char(0);
		// last_length = 0;
		history = 0;
	}
}

inline static void 
finish_input()
{
	if (echoing) {
		for (; cur < tail; cputchar(buf[cur++]))
			/* move the cursor to the tail */;
		cputchar('\n');
	}
	cur = tail;
	buf[tail] = 0;
	if (cur){
		memmove(last, buf, BUFLEN);
		last_length = tail;
		history = 0;
	}
}

#define EXT_ASCII_LF 228
#define EXT_ASCII_RT 229
#define EXT_ASCII_UP 226
#define EXT_ASCII_DN 227

inline static void 
ext_char_handler(int c)
{
	switch(c) {
	case EXT_ASCII_LF:
		move_left();
		return;
	case EXT_ASCII_RT: 
		move_right();
		return;
	case EXT_ASCII_UP:
		move_up();
		return;
	case EXT_ASCII_DN:
		move_down();
		return;
	}
	insert_char(c);
}

#define ESC_LF 'D'
#define ESC_RT 'C'
#define ESC_UP 'A'
#define ESC_DN 'B'

inline static void 
esc_seq_handler()
{
	char a, b = 0;

	a = getchar();
	if (a == '[') {
		switch(b = getchar()) {
		case ESC_LF: 
			move_left();
			return;
		case ESC_RT:
			move_right();
			return;
		case ESC_UP:
			move_up();
			return;
		case ESC_DN:
			move_down();
			return;
		}
	}
	insert_char(a);
	if (b)
		insert_char(b);
}